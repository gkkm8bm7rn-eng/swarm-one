...
if 'const APP_VERSION = "0.8.0";' not in s:
    s = s.replace('const APP_VERSION = "0.7.2";', 'const APP_VERSION = "0.8.0";')
    s = s.replace(
...
const visionRequests = new Map();''',
    ...
    marker = 'function attachmentSummary() {'
...
'''
    ...
    summary = r'''function attachmentSummary() {
...
'''
    s = replace_function(s, 'function attachmentSummary() {', '\nasync function submitPrompt', summary)

    submit = r'''async function submitPrompt(prefill) {
...
'''
    s = replace_function(s, 'async function submitPrompt(prefill) {', '\nfunction isTextFile', submit)

    old_image = 'pendingAttachments.push({ id: uid(), kind: "image", name: file.name, size: normalized.blob.size, originalSize: file.size, width: normalized.width, height: normalized.height, supported: false, text: "", previewUrl: normalized.objectUrl });'
    new_image = 'pendingAttachments.push({ id: uid(), kind: "image", name: file.name, size: normalized.blob.size, originalSize: file.size, width: normalized.width, height: normalized.height, supported: false, text: "", blob: normalized.blob, previewUrl: normalized.objectUrl });'
    if old_image not in s:
        raise RuntimeError("Expected image attachment line not found")
    ...
    s = s.replace('toast("Офлайн-модель готова");', 'toast("Основная модель готова");')
    status_old = '["WebGPU", "gpu" in navigator ? "доступен" : "нет"], ["Модель", modelReady ? MODEL_ID : "не загружена"],'
    status_new = '["WebGPU", "gpu" in navigator ? "доступен" : "нет"], ["Модель", modelReady ? MODEL_ID : "не загружена"],\n    ["Vision", visionReady ? VISION_MODEL_ID : (visionLoading ? "загружается" : "по требованию")],'
    if status_old not in s:
        raise RuntimeError("Expected status row not found")
    s = s.replace(status_old, status_new)
    p.write_text(s, encoding="utf-8")

idx = Path("index.html")
t = idx.read_text(encoding="utf-8")
t = t.replace("SWARM One · v0.7.1", "SWARM One · v0.8.0")
t = t.replace("./app.js?v=0.7.1", "./app.js?v=0.8.0")
idx.write_text(t, encoding="utf-8")

sw = Path("service-worker.js")
q = sw.read_text(encoding="utf-8")
q = q.replace("swarm-one-v0.7.1", "swarm-one-v0.8.0")
q = q.replace("./app.js?v=0.7.1", "./app.js?v=0.8.0")
if '"./vision-worker.js?v=0.8.0"' not in q:
    q = q.replace('"./app.js?v=0.8.0",', '"./app.js?v=0.8.0",\n  "./vision-worker.js?v=0.8.0",')
sw.write_text(q, encoding="utf-8")

print("Vision integration patch complete")
